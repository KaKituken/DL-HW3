...
class Critic(nn.Module):
    def __init__(self, img_shape, feature_dim, dim_factor):
        ...
        self.model = nn.Sequential(
            nn.Linear(C*H*W, feature_dim*dim_factor[0]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(feature_dim*dim_factor[0], feature_dim*dim_factor[1]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(feature_dim*dim_factor[1], feature_dim*dim_factor[2]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(feature_dim*dim_factor[2], 1)
        )
...
class Generator(nn.Module):
    def __init__(self, z_dim, img_shape, feature_dim, dim_factor) -> None:
        ...
        C, H, W = img_shape
        self.model = nn.Sequential(
            nn.Linear(z_dim, feature_dim*dim_factor[0]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm1d(feature_dim*dim_factor[0]),
            nn.Linear(feature_dim*dim_factor[0], feature_dim*dim_factor[1]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm1d(feature_dim*dim_factor[1]),
            nn.Linear(feature_dim*dim_factor[1], feature_dim*dim_factor[2]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm1d(feature_dim*dim_factor[2]),
            nn.Linear(feature_dim*dim_factor[2], feature_dim*dim_factor[3]),
            nn.LeakyReLU(0.2, inplace=True),
            nn.BatchNorm1d(feature_dim*dim_factor[3]),
            nn.Linear(feature_dim*dim_factor[3], C*H*W),
            nn.Tanh()
        )
...
if __name__ == '__main__':
    ...
    D = Critic(img_shape, feature_dim, dim_factor)
...
#             nn.Conv2d(last_feature_dim, 1, size, 1, 0, bias=False)  # remove the sigmoid activation
#         )
